...
use crate::repo::{EmailVerificationTokenRepo, SessionRepo};
...
impl CleanupService {
    ...
    pub async fn cleanup_expired_email_tokens(pool: &PgPool) -> Result<i64> {
        ...
        if deleted_count > 0 {
            info!("Cleaned up {} expired email verification tokens", deleted_count);
        }
        Ok(deleted_count)
    }
    ...
    pub async fn run_full_cleanup(pool: &PgPool) -> Result<(i64, i64)> {
        ...
        info!(
            "Full cleanup completed: {} sessions, {} email tokens",
            sessions_cleaned, email_tokens_cleaned
        );

        Ok((sessions_cleaned, email_tokens_cleaned))
    }
}
