...
pub fn build_app_with_state(state: Arc<AppState>) -> Router {
    use crate::middleware::{
        create_cors_layer, create_rate_limiter, csrf_middleware, ensure_connect_info_middleware,
        rate_limit_middleware, request_context_middleware, security_headers_middleware, CorsConfig,
    };
    ...
    Router::new()
        ...
        .route(
            ...
            post(handler::auth::reset_password),
        )
        .route("/api/v1/auth/session", get(handler::auth::get_session))
        // Passkey
        .route("/api/v1/passkey", get(handler::passkey::list_passkeys))
        .route(
            "/api/v1/passkey/register/start",
            post(handler::passkey::start_register),
        )
        .route(
        ...
        .route(
            ...
            post(handler::passkey::start_login),
        )
        .route(
            "/api/v1/passkey/login/finish",
            post(handler::passkey::finish_login),
        )
        ...
        .route(
            "/api/v1/me/apps/revoked",
            get(handler::me::get_revoked_apps),
        )
        .route("/api/v1/me/audit-logs", get(handler::me::get_audit_logs))
        .route("/api/v1/me/consents", get(handler::me::get_consents))
        .route(
            "/api/v1/me/consents/{client_id}",
            delete(handler::me::revoke_consent),
        )
        .route(
            "/api/v1/me/consents/{client_id}",
            post(handler::me::restore_consent),
        )
        .route(
            "/api/v1/me/email/change-request",
            post(handler::me::request_email_change),
        )
        .route(
            "/api/v1/me/email/verify",
            post(handler::me::verify_email_change),
        )
        // Admin
        ...
        .route(
            ...
            patch(handler::admin::update_user),
        )
        .route(
            "/api/v1/admin/users/{id}/reset-password",
            post(handler::admin::reset_user_password),
        )
        ...
        .route(
            ...
            patch(handler::admin::update_group),
        )
        .route(
            "/api/v1/admin/groups/{id}",
            delete(handler::admin::delete_group),
        )
        ...
        .route(
            ...
            patch(handler::admin::update_client),
        )
        .route(
            "/api/v1/admin/clients/{id}",
            delete(handler::admin::delete_client),
        )
        .route(
            "/api/v1/admin/clients/{id}/reset-secret",
            post(handler::admin::reset_client_secret),
        )
        .route(
            "/api/v1/admin/audit-logs",
            get(handler::admin::get_audit_logs),
        )
        .route("/api/v1/admin/settings", get(handler::admin::get_settings))
        .route(
            "/api/v1/admin/settings",
            patch(handler::admin::update_settings),
        )
        .route(
            "/api/v1/admin/keys/rotate",
            post(handler::admin::rotate_key),
        )
        // Public config (for login page branding)
        .route(
            "/api/v1/public/config",
            get(handler::auth::get_public_config),
        )
        // OIDC
        .route(
            "/.well-known/openid-configuration",
            get(handler::oidc::discovery),
        )
        ...
        // 请求流程（从外到内）：
        // security_headers → CORS → request_context → connect_info → rate_limit → csrf → handler
        .layer(axum::middleware::from_fn(security_headers_middleware))
        .layer(axum::middleware::from_fn(csrf_middleware))
        .layer(axum::middleware::from_fn(rate_limit_middleware))
        .layer(axum::middleware::from_fn(ensure_connect_info_middleware))
