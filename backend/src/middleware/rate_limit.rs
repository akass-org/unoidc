...
pub async fn rate_limit_middleware(
    ...
    let ip = extract_client_ip(request.headers(), remote_addr.as_deref());
...
mod tests {
    ...
    #[test]
    fn test_extract_ip_xff() {
        let mut h = axum::http::HeaderMap::new();
        h.insert("x-forwarded-for", "1.2.3.4, 5.6.7.8".parse().unwrap());
        assert_eq!(extract_client_ip(&h, Some("10.0.0.1:12345")), "1.2.3.4");
    }
    ...
    fn test_extract_ip_fallback_remote() {
        ...
        assert_eq!(extract_client_ip(&h, Some("10.0.0.1:12345")), "10.0.0.1");
    }
