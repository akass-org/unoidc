...
pub(crate) async fn build_login_session_response(
    state: &Arc<AppState>,
    ...
    // Session fixation protection: destroy old session before creating new one
    ...
    let session_cookie = build_cookie_value(
        &session.session_id,
        state.config.cookie_domain.as_ref(),
        secure,
...
pub(crate) fn extract_client_ip_secure(
    headers: &HeaderMap,
...
pub async fn logout(
    ...
    let session_id = extract_session_cookie(&headers, &state.config.session_secret).ok_or(
...
pub async fn forgot_password(
    ...
    if let Ok(Some(user)) = crate::repo::UserRepo::find_by_email(&state.db, &req.email).await {
        // 撤销该用户之前的所有重置令牌
        let _ = crate::repo::PasswordResetTokenRepo::revoke_all_for_user(&state.db, user.id).await;
        ...
        let expires_at =
            time::OffsetDateTime::now_utc() + time::Duration::minutes(PASSWORD_RESET_TTL_MINUTES);
...
/// 获取公共配置（无需登录，用于登录页）
pub async fn get_public_config(State(state): State<Arc<AppState>>) -> Result<Response> {
    // 从数据库读取设置
    let settings =
