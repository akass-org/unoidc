...
async fn test_successful_login() {
    ...
    let updated_user = UserRepo::find_by_id(&state.db, user.id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_account_lockout_after_repeated_failures() {
    ...
    let locked_user = UserRepo::find_by_id(&state.db, user.id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_login_creates_session() {
    ...
    let sessions = SessionRepo::find_user_sessions(&state.db, user.id)
        .await
        .unwrap();
...
async fn test_logout_destroys_session() {
    ...
    let deleted_session = SessionRepo::find_by_session_id(&state.db, &session.session_id)
        .await
        .unwrap();
