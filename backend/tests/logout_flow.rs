...
async fn test_rp_initiated_logout_success(pool: PgPool) -> anyhow::Result<()> {
    ...
    let session = SessionRepo::create(&pool, backend::model::CreateSession {
        user_id: user.id,
...
async fn test_rp_initiated_logout_with_id_token_hint(pool: PgPool) -> anyhow::Result<()> {
    ...
    let _session = SessionRepo::create(&pool, backend::model::CreateSession {
        user_id: user.id,
        ip_address: Some("127.0.0.1".to_string()),
        user_agent: Some("test-agent".to_string()),
        duration_seconds: 3600,
...
async fn test_consent_revoke_removes_refresh_tokens(pool: PgPool) -> anyhow::Result<()> {
    ...
    ConsentRepo::create(&pool, backend::model::CreateConsent {
        user_id: user.id,
        client_id: client.id,
...
async fn test_logout_all_user_sessions(pool: PgPool) -> anyhow::Result<()> {
    ...
    let session1 = SessionRepo::create(&pool, backend::model::CreateSession {
        user_id: user.id,
        ip_address: Some("127.0.0.1".to_string()),
    ...
    let session2 = SessionRepo::create(&pool, backend::model::CreateSession {
        user_id: user.id,
...
async fn test_validate_post_logout_redirect_uri_valid(pool: PgPool) -> anyhow::Result<()> {
    ...
    let result = LogoutService::validate_post_logout_redirect(&pool, &client.id, "https://example.com/").await;
    assert!(result.is_ok());
...
async fn test_validate_post_logout_redirect_uri_invalid(pool: PgPool) -> anyhow::Result<()> {
    ...
    let result = LogoutService::validate_post_logout_redirect(&pool, &client.id, "https://evil.com/callback").await;
    assert!(result.is_err());
...
#[sqlx::test]
async fn test_validate_post_logout_redirect_uri_missing_for_client(pool: PgPool) -> anyhow::Result<()> {
    let client = ClientRepo::create(&pool, CreateClient {
    ...
    let result = LogoutService::validate_post_logout_redirect(&pool, &client.id, "https://example.com/callback").await;
    assert!(result.is_err());
...
async fn create_test_user(pool: &PgPool, username: &str) -> anyhow::Result<User> {
    ...
    UserRepo::create(pool, backend::model::CreateUser {
    ...
    }).await.map_err(|e| anyhow::anyhow!("Failed to create user: {}", e))
}

async fn create_test_client(pool: &PgPool) -> anyhow::Result<Client> {
    ClientRepo::create(pool, CreateClient {
        ...
        response_types: vec!["code".to_string()],
        token_endpoint_auth_method: "client_secret_post".to_string(),
    }).await.map_err(|e| anyhow::anyhow!("Failed to create client: {}", e))
}
