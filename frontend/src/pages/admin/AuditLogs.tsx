...
import { Card, Badge, EmptyState, Table } from "#src/components/ui";
...
export function AdminAuditLogs() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (log: AuditLog) => (
        <span
          className="block max-w-[200px] truncate whitespace-nowrap text-xs text-gray-600"
    ...
    },
    {
      ...
      render: (log: AuditLog) => (
        <code
          className="block max-w-[170px] truncate whitespace-nowrap text-[11px] text-gray-600 font-mono"
      ...
      {filteredLogs.length > 0 && (
        <div className="flex items-center justify-between gap-2">
          <p className="text-xs text-gray-600">
            显示 {pageStart + 1}-{Math.min(pageStart + pageSize, filteredLogs.length)} 条，共{" "}
