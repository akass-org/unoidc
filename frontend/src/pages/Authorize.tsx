...
import { Shield, Check, X, User, Mail, Users, Key } from "lucide-react";
...
export function AuthorizePage() {
  ...
  const loadClientInfo = async () => {
    try {
      ...
      const scopes =
        preview.scopes.length > 0 ? preview.scopes : preview.scope.split(" ").filter(Boolean);
  ...
  const handleApprove = async () => {
    ...
    setProcessing(true);
    try {
      await submitApprovedAuthorization(
        consentRequest.client_id,
        consentRequest.redirect_uri,
        selectedScopes,
      );
    } catch (err) {
      addToast({
      ...
      <div className="max-w-md mx-auto">
        ...
        <Card className="overflow-hidden">
          ...
          <div className="p-6 text-center border-b border-gray-200 dark:border-white/[0.06]">
            <div className="w-16 h-16 mx-auto mb-4 rounded-xl bg-gray-100 dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] flex items-center justify-center">
              {consentRequest?.client_logo ? (
                <img src={consentRequest.client_logo} alt="" className="w-9 h-9 object-contain" />
              ) : (
            ...
            <p className="text-sm text-gray-500">{consentRequest?.client_description}</p>
              ...
              {consentRequest?.scopes.map((scope) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Icon
                      className={`w-4 h-4 flex-shrink-0 mt-0.5 ${isSelected ? "text-gray-600 dark:text-gray-400" : "text-gray-400 dark:text-gray-600"}`}
                    />
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p
                          className={`text-sm ${isSelected ? "text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-400"}`}
                        >
                          {config.label}
                        </p>
                        {isRequired && <Badge size="sm">必需</Badge>}
                      </div>
          ...
          <div className="px-5 pb-4 space-y-2">
            <Button onClick={handleApprove} loading={processing} className="w-full">
              同意授权
            </Button>
            <Button variant="ghost" onClick={handleDeny} disabled={processing} className="w-full">
              拒绝
            </Button>
          </div>
